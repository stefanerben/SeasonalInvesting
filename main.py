...
def main():
    ...
    # be in the consumer markets (in the S&P500 from November to March)
    ...
    # loop through all days until ending date is reached
        # if date = 1. November
...
def get_sp500_index():
    ...
    # TODO This is only demo data, change with live data if possible!
    ...
    symbols = ["AAPL", "MSFT", "AMZN", "TSLA", "GOOGL", "BRK.B", "UNH", "GOOG", "JNJ", "XOM"]
    weights = [20, 20, 10, 10, 10, 10, 5, 5, 5, 5 ]
...
def get_price_for_symbol(symbol, date="today"):
    """
    Given a stock symbol and an optional date, return the price of that stock
    If no price is defined, the method returns the current price
    """ 
    pass     
  
    
def buy_into_positions(symbols, amounts):
    """
...
def calculate_portfolio_value(date):
    ...
    # for every positon in portfolio
        # get current price
        # get amount
        # add position value to the portfolio_amount
